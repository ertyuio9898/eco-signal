...
def get_or_create_user(user_name):
    ...
    if user: user_id = user[0]
        ...
        print(f"🆕 새로운 사용자 '{user_name}'님을 등록했습니다.")
...
def add_activity_log(user_id, activity_type, points):
    try:
        ...
        conn.commit(); conn.close(); print(f"💾 DB 저장 완료: 사용자 ID {user_id}, {activity_type}, {points}점")
    except Exception as e: print(f"🚫 DB 저장 중 오류: {e}")

def check_and_award_achievements(user_id, user_name):
    try:
        conn = sqlite3.connect(DB_NAME, check_same_thread=False); conn.row_factory = sqlite3.Row; cursor = conn.cursor()
        ...
        for achievement in all_achievements:
            if achievement['id'] not in earned_badge_ids:
                ...
                if is_achieved:
                    ...
                    conn.commit(); print(f"🎉 뱃지 획득! '{user_name}'님이 '{achievement['badge_name']}' 뱃지를 획득했습니다!")
        conn.close()
...
def get_user_history(user_id, limit=20):
    try:
        ...
        history_data = [dict(row) for row in cursor.fetchall()]; conn.close(); return history_data
...
def get_user_achievements(user_id):
    try:
        ...
        badges_data = [dict(row) for row in cursor.fetchall()]; conn.close(); return badges_data
...
def get_recent_activities(limit=10):
    try:
        ...
        activities = [dict(row) for row in cursor.fetchall()]; conn.close(); return activities
...
def get_all_users():
    try:
        ...
        users = [row['user_name'] for row in cursor.fetchall()]; conn.close(); return users
