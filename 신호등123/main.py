...
def api_ranking():
    ...
    conn = get_db_connection()
    query = """
        SELECT user_name, SUM(co2_saved) as total_co2
        FROM tag_logs
    ...
    top_users = conn.execute(query, params).fetchall()
    conn.close()

    # Chart.js가 쓰기 좋은 형태로 데이터 가공
    ranking_data = {
        "labels": [user['user_name'] for user in top_users],
        "data": [user['total_co2'] for user in top_users]
    }
